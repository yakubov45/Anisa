        ...
        <div className="p-4 md:p-10 max-w-6xl mx-auto space-y-10 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    <h1 className="text-3xl font-black text-foreground uppercase tracking-tighter">{t('flash_management')}</h1>
                    <p className="text-surface-500 text-xs font-bold uppercase tracking-widest">{t('flash_configure')}</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={saving}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-surface dark:bg-zinc-900 border border-border-alpha dark:border-white/5 rounded-3xl p-8 space-y-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black text-surface-400 uppercase tracking-widest">{t('flash_expiration')}</label>
                                <input
                                    type="datetime-local"
                                    ...
                                    className="w-full bg-surface-100 dark:bg-white/5 border border-border-alpha dark:border-white/10 rounded-xl px-4 py-4 text-xs font-mono tracking-widest outline-none focus:ring-1 focus:ring-primary/40 text-foreground dark:text-white"
                                />
                            </div>

                            {/* Discount Percentage */}
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black text-surface-400 uppercase tracking-widest">{t('flash_global_discount')}</label>
                        ...
                        <p className="text-[9px] text-zinc-500 italic leading-relaxed">{t('flash_warning')}</p>
                    </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                <div className="relative">
                                    <input 
                                        ...
                                        className="bg-surface-100 dark:bg-white/5 border border-border-alpha dark:border-white/10 rounded-xl px-4 py-2 text-[9px] font-mono tracking-widest outline-none focus:ring-1 focus:ring-primary/40 w-full sm:w-40 text-foreground dark:text-white"
                                    />
                                </div>
                                <select 
                                    value={selectedCategory}
                                    onChange={(e) => setSelectedCategory(e.target.value)}
                                    className="bg-surface-100 dark:bg-zinc-800 border border-border-alpha dark:border-white/10 rounded-xl px-4 py-2 text-[9px] font-mono tracking-widest outline-none focus:ring-1 focus:ring-primary/40 text-foreground dark:text-white"
                                >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin">
                            {filteredProducts.length > 0 ? (
                                        ...
                                        <div
                                            ...
                                            className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-4 group ${
                                                isSelected 
                                                    ...
                                                    : 'bg-surface-100/50 dark:bg-white/5 border-border-alpha dark:border-white/10 hover:border-surface-300'
                                            }`}
                                        >
                                            <div className="w-16 h-16 bg-surface-200 dark:bg-white/10 rounded-xl overflow-hidden p-2 flex items-center justify-center">
                                ...
                                <div className="col-span-full py-20 text-center space-y-4 opacity-50">
                                    <p className="text-[10px] font-black uppercase tracking-[0.3em]">{t('flash_no_units')}</p>
                                </div>
                            )}
                        </div>
                    </div>
