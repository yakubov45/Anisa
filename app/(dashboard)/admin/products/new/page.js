...
export default function NewProductPage() {
    ...
    // Variants List
    // Format: { id, colorName, colorHex, price, stock, sku, images: [] }
    ...
    useEffect(() => {
        ...
        if (queryParams.get('import') === 'true') {
            try {
                const stored = JSON.parse(localStorage.getItem('pendingImports'));
                if (Array.isArray(stored) && stored.length > 0) {
                    setPendingImports(stored);
                    setImportCount(stored.length);
                    loadImportedData(stored[0]);
                }
            } catch(e) { console.error(e) }
        }
    }, []);

    const loadImportedData = (item) => {
        setForm(prev => ({
            ...prev,
            name: item.name || "",
        ...
        if (item.variants && item.variants.length > 0) {
            ...
            setVariants(item.variants.map((v, i) => ({ 
                ...v, 
                id: i.toString(),
                images: v.images || []
            })));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (pendingImports.length > 0) {
                ...
                newPending.shift(); // Remove the one we just saved
                
                if (newPending.length > 0) {
                    // Load the next one
                    localStorage.setItem('pendingImports', JSON.stringify(newPending));
                    setPendingImports(newPending);
                    loadImportedData(newPending[0]);
                    alert("Saqlandi! Navbatdagi mahsulot yuklanmoqda...");
                } else {
                    // Finished all
                    localStorage.removeItem('pendingImports');
                    alert("Barcha import qilingan mahsulotlar muvaffaqiyatli saqlandi!");
        ...
        <div className="max-w-6xl space-y-12 animate-fade-in pb-20">
            <div className="flex items-center justify-between">
                <div className="space-y-2">
                    <h1 className="text-4xl font-black text-surface-900 tracking-tighter uppercase">
                        {pendingImports.length > 0 ? "Import Processing" : "Initialize_Hardware"}
                    </h1>
                    <p className="text-surface-500 font-bold italic">
                        {pendingImports.length > 0 
                            ? `Reviewing imported product ${importCount - pendingImports.length + 1} of ${importCount}` 
                            : "Register a new performance unit into the global inventory."}
                    </p>
                </div>
                <div className="flex flex-col items-end gap-2">
                    {pendingImports.length > 0 && (
                        <div className="bg-primary/10 text-primary px-4 py-2 rounded-lg font-black text-xs uppercase tracking-widest animate-pulse">
                            {pendingImports.length} ta qoldi
                        </div>
                    )}
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                    ...
                    >
                        {loading ? "Processing..." : (pendingImports.length > 1 ? "Save & Next Product" : "Deploy Hardware")}
                    </button>
                </div>
            </form>
        </div>
