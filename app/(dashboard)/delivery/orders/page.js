...
import { getDeliveryOrdersAction, updateOrderAction } from "@/lib/actions/order.actions"
        ...
        async function fetchOrders() {
            ...
            try {
                if (tab === "my") {
                    ...
                    const result = await getDeliveryOrdersAction(null, "available")
                    if (result.success) {
                        setOrders(result.orders.filter(o => !o.deliveryId))
                    }
                }
            } catch (error) {
            ...
            <div className="flex gap-4 border-b border-white/5 pb-4">
                {[
                    ...
                    <button
                        ...
                        className={`text-[10px] font-black uppercase tracking-widest px-6 py-3 rounded-xl transition-all ${tab === tabItem.id ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
                    >
                        {tabItem.label}
                    </button>
                ))}
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest ${order.status?.toLowerCase() === ORDER_STATUS.SHIPPED ? 'bg-blue-500/20 text-blue-500' : 'bg-green-500/20 text-green-500'}`}>
                                            {t(`ord_status_${order.status?.toLowerCase()}`) || order.status}
                                        </div>
                                    </div>
                                    <span className="text-[9px] font-black text-white/40 uppercase tracking-widest">
                                        {order.createdAt ? new Date(typeof order.createdAt === 'string' ? order.createdAt : order.createdAt.toDate?.() || Date.now()).toLocaleDateString() : '-'}
                                    </span>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-[10px] font-bold text-white/40 uppercase tracking-wider mt-1">
                                            {order.shippingAddress?.address || order.customer?.address || order.address || "No Address"}, {order.shippingAddress?.city || order.customer?.city || order.city || ""}
                                        </p>
                            ...
                            <div className="flex flex-row md:flex-col items-center gap-3 w-full md:w-auto">
                                {tab === "available" ? (
                                    <button
                                        ...
                                        className="flex-1 md:w-full bg-primary text-white font-black px-8 py-4 rounded-xl uppercase text-[10px] tracking-widest hover:bg-white hover:text-black transition-all shadow-xl shadow-primary/20 disabled:opacity-50"
                                    >
                                        {actionLoading === order.id ? "..." : t('del_accept')}
                                    </button>
                                ) : (
                                    <>
                                        ...
                                        <a href={`tel:${order.shippingAddress?.phone || order.customer?.phone}`} className="w-14 h-14 flex items-center justify-center bg-green-600/10 text-green-500 border border-green-500/20 rounded-xl hover:bg-green-600 hover:text-white transition-all shrink-0">
