...
export default function Navbar() {
    ...
    return (
        <>
            <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 transform ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'} ${isScrolled ? 'py-3' : 'py-5'}`}>
                <div className={`mx-auto max-w-7xl px-4 md:px-8`}>
                    <div className={`bg-[#161B22]/90 backdrop-blur-3xl rounded-2xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center justify-between px-6 md:px-8 py-3.5 transition-all duration-500`}>

                        {/* LOGO */}
                        <Link href="/" className="flex items-center group shrink-0">
                            <div className="relative w-12 h-12 md:w-20 md:h-12 flex items-center justify-center transition-all overflow-hidden rounded-xl">
                        ...
                        <div className="flex items-center gap-4 lg:gap-6">
                            <div className="flex items-center gap-4 lg:gap-6 border-r border-white/10 pr-4 lg:pr-6 hidden sm:flex">
                                {[
                                    ...
                                    <Link key={link.name} href={link.href} className={`text-[9px] font-black uppercase tracking-[0.25em] transition-colors ${pathname === link.href ? 'text-primary' : 'text-white/70 hover:text-white'}`}>
                                        {link.name}
                                    </Link>
                                ))}
                            </div>
                            ...
                            <div className="flex items-center gap-3 lg:gap-5">
                                ...
                                <button
                                    ...
                                    className="h-9 px-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-2 hover:bg-primary hover:text-white transition-all group text-white"
                                >
                                    <span className="text-[10px] font-black tracking-widest">{currency}</span>
                                    <div className="w-[1px] h-3 bg-white/10 group-hover:bg-white/20" />
                                    <span className="text-[10px] font-black opacity-40 group-hover:opacity-100">{currency === 'USD' ? 'UZS' : 'USD'}</span>
                                </button>
                                            ...
                                            {isProfileOpen && (
                                                <div className="absolute right-0 mt-4 w-56 bg-[#161B22] border border-white/10 rounded-2xl shadow-premium p-2 animate-slide-up z-[60]">
                                                    ...
                                                    <div className="space-y-1">
                                                        ...
                                                        >
                                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> Profile
                                                        </Link>
                                                        ...
                                                        {user.role === 'admin' && (
                                                            ...
                                                            >
                                                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg> Admin Panel
                                                            </Link>
                                                        )}
                                                        ...
                                                        {user.role === 'delivery' && (
                                                            ...
                                                            >
                                                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1m-4 0h4"/></svg> Delivery Panel
                                                            </Link>
                                                        )}
                                                        ...
                                                        >
                                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg> Log Out
                                                        </button>
                                                    </div>
                                                </div>
                                            )}
            ...
            <div className={`fixed inset-0 z-[100] lg:hidden transition-all duration-500 ${isMobileMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
                ...
                {/* Drawer Content */}
                <div className={`absolute right-0 top-0 bottom-0 w-full max-w-sm bg-[#050505] shadow-2xl transition-transform duration-500 z-[101] flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                    {/* Header */}
                    ...
                    <div className="flex border-b border-white/5 bg-[#050505]">
                        <Link
                            href="/wishlist"
                            onClick={() => setIsMobileMenuOpen(false)}
                            className="flex-1 flex items-center justify-center gap-3 py-6 text-white hover:bg-white/5 transition-all border-r border-white/5"
                        >
                        ...
                        <Link
                            href="/cart"
                            onClick={() => setIsMobileMenuOpen(false)}
                            className="flex-1 flex items-center justify-center gap-3 py-6 text-white hover:bg-white/5 transition-all"
                        >
                            <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar bg-black">
                        <div className="flex flex-col">
                            <Link
                                href="/faq"
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="flex items-center justify-between px-8 py-6 border-b border-white/10 bg-primary/5 hover:bg-primary/10 transition-all group"
                            >
                                <div className="flex items-center gap-4">
                                    <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    <span className="text-[11px] font-black text-primary uppercase tracking-[0.2em]">FAQ & Support</span>
                                </div>
                            ...
                            <div className="p-4 bg-white/5">
                                <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] px-4 py-2">Categories</p>
                            </div>
                            ...
                            {categories.map((cat) => (
                                <Link
                                    ...
                                    onClick={() => setIsMobileMenuOpen(false)}
                                    className="flex items-center justify-between px-8 py-5 border-b border-white/5 hover:bg-white/5 transition-all group"
                                >
                                    <span className="text-[11px] font-bold text-white uppercase tracking-wider group-hover:text-primary transition-colors">
                                        {cat.name}
                                    </span>
                                    <svg className="w-4 h-4 text-white opacity-20 group-hover:opacity-100 transition-all group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="p-8 border-t border-white/5 bg-[#050505]">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[9px] font-black text-white/40 uppercase tracking-widest">System Status: Online</span>
                        </div>
                        <p className="text-[8px] font-black text-white/20 uppercase tracking-[0.3em]">OnePC Engineering 2026</p>
                    </div>
                </div>
            </div>
        </>
