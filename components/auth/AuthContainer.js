...
import { checkPhoneStatusAction } from "@/lib/actions/user.actions";
...
export default function AuthContainer({ initialMode = "login" }) {
    ...
    const [phoneMode, setPhoneMode] = useState(false); // Toggle between Email and Phone
    const [otpSent, setOtpSent] = useState(false);
    const [otp, setOtp] = useState("");
    const [recaptchaReady, setRecaptchaReady] = useState(false);
    const [confirmationResult, setConfirmationResult] = useState(null);
    const [showPhonePasswordField, setShowPhonePasswordField] = useState(false);
    const [linkedEmail, setLinkedEmail] = useState(null);
    const router = useRouter();

    useEffect(() => {
        if (typeof window !== "undefined" && !recaptchaReady) {
            authService.setupRecaptcha("recaptcha-container").then(() => {
    ...
    const handleChange = (e) => {
        ...
        if (name === "phoneNumber") {
            // Faqat raqamlar va + belgisini qoldiramiz, bo'shliqlarni o'chiramiz
                ...
                if (!otpSent) {
                    if (mode === "login") {
                        ...
                        if (formData.password) {
                            ...
                            const status = await checkPhoneStatusAction(cleanPhone);
                            const emailToUse = (status.success && status.email) ? status.email : null;
                            
                            await authService.loginWithPhonePassword(cleanPhone, formData.password, emailToUse);
                            router.push("/");
                            return;
                        } else {
                            throw new Error("Iltimos, parolingizni kiriting yoki SMS orqali kiring.");
                        }
                    }

                    if (mode === "register") {
                        // Validate registration fields before sending SMS
                        if (!formData.name || !formData.password) {
                            throw new Error("Iltimos, ism va parolni kiriting.");
                        }
                    ...
                    if (mode === "register") {
                        ...
                        await authService.completePhoneRegistration(result.user, formData.name, formData.password);
                    }
        ...
        <div className="w-full max-w-[480px] bg-white dark:bg-zinc-900 p-6 md:p-14 rounded-3xl md:rounded-[3rem] shadow-2xl border border-surface-200 dark:border-white/10 relative overflow-hidden min-h-[500px] md:min-h-[600px] flex flex-col justify-center">
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="flex bg-surface-50 dark:bg-black p-1 rounded-2xl border border-surface-200 dark:border-white/10">
                        <button 
                            type="button"
                            onClick={() => {setPhoneMode(false); setOtpSent(false); setShowPhonePasswordField(false);}}
                            className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all ${!phoneMode ? 'bg-white dark:bg-zinc-800 shadow-sm text-primary' : 'text-surface-500'}`}
                        >
                            Email
                        </button>
                        <button 
                            type="button"
                            onClick={() => {setPhoneMode(true); setOtpSent(false); setShowPhonePasswordField(false);}}
                            className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all ${phoneMode ? 'bg-white dark:bg-zinc-800 shadow-sm text-primary' : 'text-surface-500'}`}
                        >
                            Phone
                        </button>
                        ...
                        {phoneMode ? (
                            otpSent ? (
                                <div className="space-y-4">
                                    ...
                                    <p className="text-[9px] text-surface-500 font-bold uppercase text-center">
                        ...
                        >
                            {loading ? t('auth_processing') : (
                                phoneMode 
                                    ...
                                    : (mode === "login" ? t('auth_enter_portal') : t('auth_register_now'))
                            )}
                        </button>
