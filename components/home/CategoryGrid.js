    ...
    return (
        <section className="space-y-12">
            <div className="flex items-center gap-4">
                ...
                <h2 className="text-3xl font-black text-foreground uppercase tracking-tighter">Categories</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {categories.map((cat) => (
                    <Link 
                        key={cat.name} 
                        href={cat.href}
                        className={`group relative overflow-hidden bg-[#0A0A0B] border border-white/5 p-8 rounded-[2rem] transition-all hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10`}
                    >
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-6">
                            ...
                            <span className="font-black text-[10px] uppercase tracking-[0.2em] text-white/40 group-hover:text-primary transition-colors">{cat.name}</span>
                        </div>
                    </Link>
                ))}
            </div>
