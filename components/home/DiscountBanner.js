...
export default function DiscountBanner() {
    ...
    useEffect(() => {
        if (!settings?.endTime || isExpired) return;
            ...
            <div className="relative z-10 p-6 md:p-14 space-y-10">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                    ...
                    <div className="flex gap-4">
                        <button 
                            onClick={() => scroll('left')}
                            className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-primary hover:border-primary hover:scale-110 transition-all active:scale-95 group shadow-xl"
                        >
                ...
                <div 
                    ...
                    className="flex gap-6 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-4 pt-4"
                    ...
                    {products.map((product) => (
                        <div key={product.id} className="min-w-[280px] sm:min-w-[320px] md:min-w-[360px] snap-start">
                            <ProductCard 
                                product={{
                                    ...product,
                                    discount: settings?.discountPercentage || 15
                                }} 
                                badge="FLASH" 
                            />
                        </div>
                ...
                <div className="relative w-full h-[2px] bg-white/5 rounded-full mt-6 overflow-hidden">
                    <div 
                        className="absolute h-full bg-primary transition-all duration-300 ease-out shadow-[0_0_10px_rgba(227,30,36,0.5)]"
                        style={{ 
                            width: `${Math.max(10, (1 / products.length) * 100)}%`,
                            left: `${scrollProgress * (1 - Math.max(0.1, 1 / products.length))}%`
                        }}
                    />
                </div>
            </div>
