...
export function AdminGuard({ children }) {
    ...
    useEffect(() => {
        if (!loading) {
            ...
            if (user.role === ROLES.ADMIN) {
                if (pathname.includes('/admin/users') || pathname.includes('/admin/currency')) {
    ...
    if (user.role === ROLES.ADMIN && (pathname.includes('/admin/users') || pathname.includes('/admin/currency'))) {
        return null;
    }
