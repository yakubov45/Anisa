...
export async function getPreBuiltSystemsAction(filters = {}) {
    if (!adminDb) return [];
    try {
        ...
        if (filters.isFeatured) {
            ...
            // Firebase requires a composite index if we mix where() and orderBy().
            // Skipping orderBy here prevents the crash entirely without needing an index.
        } else {
            // Only order by createdAt if we aren't using other filters
            query = query.orderBy("createdAt", "desc");
        }
