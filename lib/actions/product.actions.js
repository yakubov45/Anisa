 ...
 */
export async function getFilteredProductsAction({ categoryId, brand, sortBy = 'newest', page = 1, pageSize = 12 }) {
    if (!adminDb) return [];
    
    // Create a dynamic cache key
    const cacheKey = `filtered-${categoryId || 'all'}-${brand || 'all'}-${sortBy}-${page}-${pageSize}`;
