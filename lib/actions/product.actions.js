...
export async function getAdminProductsAction(page = 1, pageSize = 10, searchQuery = "") {
    if (!adminDb) return { products: [], total: 0 };
    try {
        if (searchQuery && searchQuery.trim().length > 0) {
            // Search mode: fetch all and filter in memory
            const snapshot = await adminDb.collection(COLLECTIONS.PRODUCTS).orderBy("createdAt", "desc").get();
            const allProducts = snapshot.docs.map(serializeDoc).filter(Boolean);
...
/**
 * Get Maximum Product Price (Cached)
 */
export async function getMaxPriceAction() {
    if (!adminDb) return 5000;
    
    const fetcher = unstable_cache(
        async () => {
            try {
                const snapshot = await adminDb.collection(COLLECTIONS.PRODUCTS)
                    .orderBy("price", "desc")
                    .limit(1)
                    .get();
                if (snapshot.empty) return 5000;
                return Number(snapshot.docs[0].data().price) || 5000;
            } catch (error) {
 ...
 */
export async function getFilteredProductsAction({ categoryId, brand, minPrice, maxPrice, sortBy = 'newest', page = 1, pageSize = 12 }) {
    if (!adminDb) return [];
    
    // Create a dynamic cache key
    const cacheKey = `filtered-${categoryId || 'all'}-${brand || 'all'}-${minPrice || 0}-${maxPrice || 'max'}-${sortBy}-${page}-${pageSize}`;
    ...
    const fetcher = unstable_cache(
        async () => {
            try {
                ...
                // We fetch a larger limit to safely sort in memory if categoryId, brand, or discount sorting is present
                const fetchLimit = (normalizedCategoryId || brand || sortBy === 'discount') ? 5000 : pageSize;
                const offset = (normalizedCategoryId || brand || sortBy === 'discount') ? 0 : (page - 1) * pageSize;
                ...
                // Filter by brand manually in JS (case-insensitive)
                if (brand) {
                    const lowerBrand = brand.toLowerCase();
                    products = products.filter(p => p.brand && p.brand.toLowerCase() === lowerBrand);
                }

                // Filter by price manually in JS
                ...
                if (normalizedCategoryId || brand || (minPrice !== undefined) || sortBy === 'discount') {
                    if (sortBy === 'discount') {
 ...
 */
export async function getProductsCountAction(options = {}) {
    if (!adminDb) return 0;
    
    const { categoryId = null, brand = null, sortBy = null, minPrice, maxPrice } = options;
    const normalizedCategoryId = normalizeCategory(categoryId);
    const cacheKey = `products-count-${normalizedCategoryId || 'all'}-${brand || 'all'}-${sortBy || 'none'}-${minPrice || 0}-${maxPrice || 'max'}`;
    ...
    const fetcher = unstable_cache(
        async () => {
            try {
                ...
                if (brand || sortBy === 'discount' || (minPrice !== undefined)) {
                    ...
                    if (brand) {
                        const lowerBrand = brand.toLowerCase();
                        items = items.filter(p => p.brand && p.brand.toLowerCase() === lowerBrand);
                    }
                    if (sortBy === 'discount') {
                        items = items.filter(p => (p.discount || 0) > 0);
                    }
                    if (minPrice !== undefined && maxPrice !== undefined) {
...
export async function createBrandAction(data) {
    if (!adminDb) return { success: false, error: "DB yopiq" };
    try {
        const docRef = adminDb.collection(COLLECTIONS.BRANDS).doc();
        const brandData = {
            ...data,
            createdAt: new Date().toISOString()
        };
        await docRef.set(brandData);
        revalidateTag('brands'); // We might want to tag brands
...
export async function deleteBrandAction(id) {
    if (!adminDb) return { success: false, error: "DB yopiq" };
    try {
        await adminDb.collection(COLLECTIONS.BRANDS).doc(id).delete();
...
export async function updateBrandAction(id, data) {
    if (!adminDb) return { success: false, error: "DB yopiq" };
    try {
        await adminDb.collection(COLLECTIONS.BRANDS).doc(id).set(
...
const getCachedAllProductsForSearch = unstable_cache(
    async () => {
        if (!adminDb) return [];
        try {
            const snapshot = await adminDb.collection(COLLECTIONS.PRODUCTS).limit(1000).get();
            return snapshot.docs.map(serializeDoc).filter(Boolean);
        } catch (error) {
            console.error("Error fetching all products for search cache:", error);
            return [];
        }
    },
    ["all-products-for-search"],
    { revalidate: 600, tags: ["products"] } // 10 minutes cache
);

export async function getSearchProductsAction(searchQuery = "", limit = 50) {
    try {
        const products = await getCachedAllProductsForSearch();
...
export async function getKitchenProductsAction(category = "all") {
    if (!adminDb) return [];
    try {
        let query = adminDb.collection(COLLECTIONS.PRODUCTS);
