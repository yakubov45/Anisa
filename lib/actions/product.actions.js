    ...
    const fetcher = unstable_cache(
        async () => {
            try {
                ...
                const fetchLimit = (normalizedCategoryId || brand) ? 200 : pageSize;
                const offset = (normalizedCategoryId || brand) ? 0 : (page - 1) * pageSize;
                ...
                if (normalizedCategoryId || brand || (minPrice !== undefined)) {
                    if (sortBy === 'price-low') {
 ...
 */
export async function getProductsCountAction(categoryId = null, brand = null) {
    if (!adminDb) return 0;
    
    const normalizedCategoryId = normalizeCategory(categoryId);
    const cacheKey = `products-count-${normalizedCategoryId || 'all'}-${brand || 'all'}`;
    ...
    const fetcher = unstable_cache(
        async () => {
            try {
                ...
                if (brand) {
                    // Agar faqat count olinadigan bo'lsa va JS da filter qilsak, barchasini olib kelib sanashimiz kerak.
                    // Chunki brand katta/kichik harf farq qilishi mumkin.
                    const snapshot = await query.get();
                    const lowerBrand = brand.toLowerCase();
                    return snapshot.docs.map(d => d.data()).filter(p => p.brand && p.brand.toLowerCase() === lowerBrand).length;
                } else {
...
export async function getBrandsAction() {
    if (!adminDb) return [];
    try {
        const query = adminDb.collection(EXTRA_COLLECTIONS.BRANDS).orderBy("name");
        const snapshot = await withTimeout(query.get(), 3000, null);
        if (!snapshot) return [];
        return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    } catch (error) {
...
export const getBannersAction = unstable_cache(
    async () => {
        if (!adminDb) return [];
        try {
            const query = adminDb.collection(EXTRA_COLLECTIONS.BANNERS).orderBy("order", "asc");
            const snapshot = await withTimeout(query.get(), 10000, null);
            if (!snapshot) return [];
            return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        } catch (error) {
...
export const getPreBuiltSystemsAction = unstable_cache(
    async (filters = {}) => {
        if (!adminDb) return [];
        try {
            let query = adminDb.collection(EXTRA_COLLECTIONS.PREBUILTS);
...
export async function getPreBuiltByIdAction(id) {
    ...
    try {
        const doc = await adminDb.collection(EXTRA_COLLECTIONS.PREBUILTS).doc(id).get();
        if (!doc.exists) return null;
...
export async function createPreBuiltSystemAction(data) {
    ...
    try {
        const newRef = adminDb.collection(EXTRA_COLLECTIONS.PREBUILTS).doc();
        await newRef.set({
            ...data,
...
export async function updatePreBuiltSystemAction(id, data) {
    ...
    try {
        await adminDb.collection(EXTRA_COLLECTIONS.PREBUILTS).doc(id).set({
            ...data,
...
export async function deletePreBuiltSystemAction(id) {
    if (!adminDb || !id) throw new Error("No database connection or invalid ID");
    try {
        await adminDb.collection(EXTRA_COLLECTIONS.PREBUILTS).doc(id).delete();
        return { success: true };
    } catch (error) {
