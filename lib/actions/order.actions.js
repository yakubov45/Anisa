    ...
    try {
        ...
        await adminDb.runTransaction(async (transaction) => {
            ...
            const productRefs = validatedCart.map(item => {
                ...
                return { item, ref: adminDb.collection(collectionName).doc(item.id) };
            });
            ...
            for (let i = 0; i < productDocs.length; i++) {
                ...
                if (productData.stock !== undefined && productData.stock < item.quantity) {
                    throw new Error(`Kechirasiz, ${item.name} omborda yetarli emas. (Qoldiq: ${productData.stock} ta)`);
                }
                ...
                if (productData.stock !== undefined) {
                    transaction.update(ref, { stock: productData.stock - item.quantity });
                }
            }
