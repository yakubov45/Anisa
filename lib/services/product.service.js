...
export const productService = {
    ...
    sanitizeProductData(data) {
        ...
        const allowedFields = ['name', 'nameRu', 'nameEn', 'description', 'descriptionRu', 'descriptionEn', 'price', 'basePrice', 'discount', 'category', 'brand', 'image', 'images', 'countInStock', 'stock', 'specs', 'status', 'variants'];
        const sanitized = {};
        
        for (const key of allowedFields) {
            if (data[key] !== undefined) {
                // Xavfli type'larni (masalan object o'rniga kelgan function yoki nosql querylar) tozalash
                if (key === 'price' || key === 'basePrice' || key === 'countInStock' || key === 'stock' || key === 'discount') {
    ...
    /**
     * Delete all products using batch (Fast bulk delete)
     */
    async deleteAll(productIds) {
        const { writeBatch, doc } = await import("firebase/firestore");
        
        // Chunk array into 500 (Firestore limit per batch)
        const chunks = [];
