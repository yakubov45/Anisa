...
export const productService = {
    ...
    sanitizeProductData(data) {
        ...
        const allowedFields = ['name', 'nameRu', 'nameEn', 'description', 'descriptionRu', 'descriptionEn', 'price', 'category', 'brand', 'image', 'images', 'countInStock', 'stock', 'specs', 'status', 'variants'];
        const sanitized = {};
        
        for (const key of allowedFields) {
            if (data[key] !== undefined) {
                // Xavfli type'larni (masalan object o'rniga kelgan function yoki nosql querylar) tozalash
                if (key === 'price' || key === 'countInStock' || key === 'stock') {
