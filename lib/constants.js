...
export const CATEGORY_TRANSLATIONS = {
    ...
    ssd: { uz: "Xotira disklari", ru: "Накопители", en: "Storage" },
    psus: { uz: "Quvvat manbalari", ru: "Блоки питания", en: "Power Supplies" },
    psu: { uz: "Quvvat manbalari", ru: "Блоки питания", en: "Power Supplies" },
    "power units": { uz: "Quvvat manbalari", ru: "Блоки питания", en: "Power Supplies" },
    "power unit": { uz: "Quvvat manbalari", ru: "Блоки питания", en: "Power Supplies" },
    cases: { uz: "Kompyuter korpuslari", ru: "Корпуса", en: "Cases" },
    case: { uz: "Kompyuter korpuslari", ru: "Корпуса", en: "Cases" },
    cooling: { uz: "Sovitish tizimlari", ru: "Системы охлаждения", en: "Cooling" },
    cooler: { uz: "Sovitish tizimlari", ru: "Системы охлаждения", en: "Cooling" },
    kullerlar: { uz: "Kullerlar", ru: "Кулеры", en: "Coolers" },
    motherboards: { uz: "Ona platalar", ru: "Материнские платы", en: "Motherboards" },
    motherboard: { uz: "Ona platalar", ru: "Материнские платы", en: "Motherboards" },
...
export function getCategoryDisplayName(category, lang = 'uz') {
    ...
    // Check CATEGORY_TRANSLATIONS map first for standard category keys (slug, id, name)
    const keysToTry = [
    ...
    for (const key of keysToTry) {
        if (key && CATEGORY_TRANSLATIONS[key]) {
            return CATEGORY_TRANSLATIONS[key][lang] || CATEGORY_TRANSLATIONS[key]['uz'] || CATEGORY_TRANSLATIONS[key]['en'];
        }
    }

    // Explicit language fields on database document
    if (lang === 'ru' && category.name_ru) return category.name_ru;
    if (lang === 'en' && category.name_en) return category.name_en;
    if (lang === 'uz' && category.name_uz) return category.name_uz;
